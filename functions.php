...
function do_page_headline($sub, $title, $p=false) {
	echo '<div class="page-default--intro">';
	...
	echo '<hr class="divider divider--red">';
	echo '</div>';
}



function get_category_newslabel($postID, $linked=false) {
	$cats = get_the_category($postID);
	$cat_names = array();
	foreach($cats as $cat) {
		$cat_names[] = $cat->cat_name;
	}
	return $cat_names = implode(', ', $cat_names);
}
...
if (function_exists('acf_add_options_page')) {
	...
	acf_set_options_page_title('EnclaraHC');
...
function mag_unregister_tags() {
    unregister_taxonomy_for_object_type('post_tag', 'post');
}
add_action('init', 'mag_unregister_tags');
...
add_image_size( 'careers_page_image', 704, 469, true );

add_image_size( 'page_full_width', 970, 550, true );
